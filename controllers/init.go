...
func Init(c *config.Config, s *service.Service) {
	...
	s.Echo.GET("/getCaptcha", GetCaptcha)
	...
	s.Echo.GET("/record/:id/comment", GetCommentList)
	s.Echo.POST("/record/:id/comment", AddComment)
	...
	admin.GET("/record/:id/review", RenderReviewRecord)
